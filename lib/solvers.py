...
def initialize_optimizer(params, config):
  assert config.optimizer in ['SGD', 'Adagrad', 'Adam', 'RMSProp', 'Rprop', 'SGDLars', 'AdamW']
...
def initialize_scheduler(optimizer, config, last_step=-1):
  if config.scheduler == 'StepLR':
    ...
    return OneCycleLR(optimizer, max_lr=config.lr, total_steps=config.max_iter+5, last_epoch=last_step)
  else:
