...
def train(model, data_loader, val_data_loader, config):
    ...
    ce_loss = CrossEntropyLoss(ignore_index=config.ignore_index)
    dice_loss = DiceLoss(ignore_index=config.ignore_index)
    ...
    for epoch in range(1, config.max_epochs+1):
        for img, target in data_loader:
            ...
            loss = ce_loss(output, target) + dice_loss(output, target)
            loss.backward()
